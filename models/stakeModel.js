...
// Create new stake
async function createStake(uid, index_no, chain, staker, amount, reward) {
    const fields = "uid, index_no, chain, staker, amount, reward, created_on, status";
    const values = `'${uid}', '${index_no}', '${chain}', '${staker}', '${amount}', '${reward}', NOW(), 'a'`;
...
async function unstake(index_no, uid, address, chain) {
    const sql = `
        UPDATE stakes 
        SET status='u', unstake_dtg=NOW() 
        WHERE staker='${address}' AND uid='${uid}' 
        AND chain='${chain}' AND index_no=${index_no}
    `;
    const [rows] = await pool.query(sql);
    return rows;
}
...
async function getMyStakes(address, chain) {
    const sql = `
        ...
        WHERE staker='${address}' AND stakes.chain='${chain}' AND stakes.status='a';
    `;
    const [rows] = await pool.query(sql);
    return rows;
}
